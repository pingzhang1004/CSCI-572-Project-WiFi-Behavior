...
RoutingExperiment::Run (double txp, std::string CSVfileName)
{
  ...
  positionAlloc2->Add (Vector (150.0, 0.0, 0.0)); 
  ...
  positionAlloc3->Add (Vector (200.0, 0.0, 0.0)); 
  mobility.SetPositionAllocator (positionAlloc3);
  mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  mobility.Install (adhocNodes.Get (3));
...
//chuang jian  xin de 
 ...
 Ptr<Socket> sink1 = SetupPacketReceive (adhocInterfaces.GetAddress (3), adhocNodes.Get (3));

    AddressValue remoteAddress1 (InetSocketAddress (adhocInterfaces.GetAddress (3), port));
    onoff1.SetAttribute ("Remote", remoteAddress1);

   ApplicationContainer temp1 = onoff1.Install (adhocNodes.Get (2));
   temp1.Start (Seconds (1.0));
 ...
 AsciiTraceHelper ascii;
  Ptr<OutputStreamWrapper> osw = ascii.CreateFileStream ("baolu-wifi-phy.tr");
  ...
  AnimationInterface anim ("baolu.xml"); 
  Simulator::Run ();
